...
public class ScanIntentIntegrator {
	...
	private static final String LOG_TAG = ScanIntentIntegrator.class
			.getSimpleName();
	...
	public ScanIntentIntegrator(Activity activity) {
		...
		title = activity.getString(R.string.error_zxing_missing_title);
		message = activity.getString(R.string.error_zxing_missing_message);
		buttonYes = activity.getString(R.string.zxing_go_get);
	 ...
	 * @param code
	 */
	public AlertDialog initiateScan(int code) {
		return initiateScan(ALL_CODE_TYPES, code);
	}
				...
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialogInterface, int i) {
						...
						try {
							...
							Log.w(LOG_TAG,
									"Google Play is not installed; cannot install Barcode Scanner");
						}
					}
				});
	 ...
	 *         else an {@link ScanIntentResult} containing the result of the
	 *         scan. If the user cancelled scanning, the fields will be null.
	 */
	public static ScanIntentResult parseActivityResult(int requestCode,
		...
		if (requestCode == REQUEST_CODE) {
			if (resultCode == Activity.RESULT_OK) {
				...
				return new ScanIntentResult(contents, formatName, rawBytes,
