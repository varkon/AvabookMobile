...
public class AuthManagerOld implements AuthManager {
	 ...
	 *            request code
	 * @param code
	 *            The request code to pass to {@link Activity#onActivityResult}
	...
	private class LoginRunnable implements Runnable {
		// @Override
		public void run() {
			GoogleLoginServiceHelper.getCredentials(activity, code, extras,
					requireGoogle, service, true);
		}
	}
	...
	public void authResult(int resultCode, Intent results) {
		...
		authToken = results
				.getStringExtra(GoogleLoginServiceConstants.AUTHTOKEN_KEY);
		if (authToken == null) {
			// Retry, without prompting the user.
			GoogleLoginServiceHelper.getCredentials(activity, code, extras,
					requireGoogle, service, false);
		} else {
	...
	public void invalidateAndRefresh(AuthCallback callback) {
		...
		activity.runOnUiThread(new Runnable() {
			public void run() {
				GoogleLoginServiceHelper.invalidateAuthToken(activity, code,
						authToken);
			}
		});
	}
	...
	public Object getAccountObject(String accountName, String accountType) {
		throw new UnsupportedOperationException(
				"Legacy auth manager knows nothing about accounts");
	}
}
