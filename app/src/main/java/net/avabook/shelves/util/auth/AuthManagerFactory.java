...
public class AuthManagerFactory {
	private static final String LOG_TAG = "AuthManageraFactory";
	...
	public static AuthManager getAuthManager(Activity activity, int code,
		...
		if (useModernAuthManager()) {
			Log.i(LOG_TAG, "Creating modern auth manager: " + service);
			return new ModernAuthManager(activity, service);
		} else {
			Log.i(LOG_TAG, "Creating legacy auth manager: " + service);
			return new AuthManagerOld(activity, code, extras, requireGoogle,
					service);
		}
	}
