...
public class ApparelProvider extends ContentProvider {
	...
	public static final String DATABASE_NAME = "apparel.db";

	private static final int DATABASE_VERSION = 5;

	private static final int SEARCH = 1;
	private static final int APPAREL = 2;
	private static final int APPAREL_ID = 3;
	private static final int LIVE_FOLDER_APPAREL = 4;

	private static final String AUTHORITY = "ApparelProvider";
	...
	static {
		...
		URI_MATCHER.addURI(AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY
				+ "/*", SEARCH);
		URI_MATCHER.addURI(AUTHORITY, "apparel", APPAREL);
		URI_MATCHER.addURI(AUTHORITY, "apparel/#", APPAREL_ID);
		URI_MATCHER.addURI(AUTHORITY, "live_folders/apparel",
				LIVE_FOLDER_APPAREL);
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case APPAREL_ID:
			...
			break;
		case LIVE_FOLDER_APPAREL: // GJT: Added this for live folder support
			qb.setTables("apparel");
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (URI_MATCHER.match(uri) != APPAREL) {
			throw new IllegalArgumentException("Unknown URI " + uri);
		}

		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		final long rowId = db.insert("apparel", BaseItem.TITLE, values);
		if (rowId > 0) {
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (URI_MATCHER.match(uri)) {
		case APPAREL:
			count = db.delete("apparel", selection, selectionArgs);
			break;
		case APPAREL_ID:
			String segment = uri.getPathSegments().get(1);
			count = db.delete("apparel", BaseItem._ID
					+ "="
					+ segment
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch (URI_MATCHER.match(uri)) {
		case APPAREL:
			count = db.update("apparel", values, selection, selectionArgs);
			break;
		case APPAREL_ID:
			count = db.update("apparel", values,
					BaseItem._ID
							+ " = "
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE apparel (" + BaseItem._ID
					...
					+ " TEXT, " + BaseItem.DEPARTMENT + " TEXT, "
					+ BaseItem.FABRIC + " TEXT, " + BaseItem.REVIEWS
			...
			db.execSQL("CREATE INDEX apparelIndexTitle ON apparel("
					+ BaseItem.SORT_TITLE + ");");
			db.execSQL("CREATE INDEX apparelIndexDirectors ON apparel("
					+ BaseItem.AUTHORS + ");");
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			switch (oldVersion) {
			case 1:
				db.execSQL("ALTER TABLE apparel ADD COLUMN "
						+ BaseItem.LOANED_TO + " TEXT");
				db.execSQL("ALTER TABLE apparel ADD COLUMN "
						+ BaseItem.LOAN_DATE + " TEXT");
				db.execSQL("ALTER TABLE apparel ADD COLUMN "
						+ BaseItem.EVENT_ID + " INTEGER");
				db.execSQL("ALTER TABLE apparel ADD COLUMN " + BaseItem.NOTES
						+ " TEXT");
			case 2:
				db.execSQL("ALTER TABLE apparel ADD COLUMN " + BaseItem.UPC
						+ " TEXT");
			case 3:
				db.execSQL("ALTER TABLE apparel ADD COLUMN "
