...
public class ToolsProvider extends ContentProvider {
	private static final String LOG_TAG = "ToolsProvider";

	public static final String DATABASE_NAME = "tools.db";
	...
	private static final int LIVE_FOLDER_TOOLS = 4;

	private static final String AUTHORITY = "ToolsProvider";
	...
	static {
		...
		URI_MATCHER.addURI(AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY
				+ "/*", SEARCH);
		URI_MATCHER.addURI(AUTHORITY, "tools", TOOLS);
		URI_MATCHER.addURI(AUTHORITY, "tools/#", TOOL_ID);
		URI_MATCHER.addURI(AUTHORITY, "live_folders/tools", LIVE_FOLDER_TOOLS);
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case TOOL_ID:
			...
			break;
		case LIVE_FOLDER_TOOLS: // GJT: Added this for live folder support
			qb.setTables("tools");
	...
	public String getType(Uri uri) {
		switch (URI_MATCHER.match(uri)) {
		case TOOLS:
			return "vnd.android.cursor.dir/vnd.biz.varkon.shelvesom.tools";
		case TOOL_ID:
			return "vnd.android.cursor.item/vnd.biz.varkon.shelvesom.tools";
		default:
			throw new IllegalArgumentException("Unknown URI " + uri);
		}
	}
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (URI_MATCHER.match(uri) != TOOLS) {
			throw new IllegalArgumentException("Unknown URI " + uri);
		}

		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		final long rowId = db.insert("tools", BaseItem.TITLE, values);
		if (rowId > 0) {
			Uri insertUri = ContentUris.withAppendedId(
					ToolsStore.Tool.CONTENT_URI, rowId);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (URI_MATCHER.match(uri)) {
		case TOOLS:
			count = db.delete("tools", selection, selectionArgs);
			break;
		case TOOL_ID:
			String segment = uri.getPathSegments().get(1);
			count = db.delete("tools", BaseItem._ID
					+ "="
					+ segment
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch (URI_MATCHER.match(uri)) {
		case TOOLS:
			count = db.update("tools", values, selection, selectionArgs);
			break;
		case TOOL_ID:
			count = db.update("tools", values,
					BaseItem._ID
							+ " = "
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE tools (" + BaseItem._ID
			...
			db.execSQL("CREATE INDEX toolIndexTitle ON tools("
					+ BaseItem.SORT_TITLE + ");");
			db.execSQL("CREATE INDEX toolIndexDirectors ON tools("
					+ BaseItem.AUTHORS + ");");
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			switch (oldVersion) {
			case 1:
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.LOANED_TO
						+ " TEXT");
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.LOAN_DATE
						+ " TEXT");
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.EVENT_ID
						+ " INTEGER");
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.NOTES
						+ " TEXT");
			case 2:
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.UPC
						+ " TEXT");
			case 3:
				db.execSQL("ALTER TABLE tools ADD COLUMN "
						+ BaseItem.WISHLIST_DATE + " TEXT");
			case 4:
				db.execSQL("ALTER TABLE tools ADD COLUMN " + BaseItem.QUANTITY
						+ " TEXT");
				break;
			default:
				break;
