...
public class ToysProvider extends ContentProvider {
	private static final String LOG_TAG = "ToysProvider";
	...
	private static final int TOYS = 2;
	private static final int TOY_ID = 3;
	private static final int LIVE_FOLDER_TOYS = 4;

	private static final String AUTHORITY = "ToysProvider";
	...
	static {
		...
		URI_MATCHER.addURI(AUTHORITY, SearchManager.SUGGEST_URI_PATH_QUERY
				+ "/*", SEARCH);
		URI_MATCHER.addURI(AUTHORITY, "toys", TOYS);
		URI_MATCHER.addURI(AUTHORITY, "toys/#", TOY_ID);
		URI_MATCHER.addURI(AUTHORITY, "live_folders/toys", LIVE_FOLDER_TOYS);
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case TOY_ID:
			...
			break;
		case LIVE_FOLDER_TOYS: // GJT: Added this for live folder support
			qb.setTables("toys");
	...
	public String getType(Uri uri) {
		switch (URI_MATCHER.match(uri)) {
		case TOYS:
			return "vnd.android.cursor.dir/vnd.biz.varkon.shelvesom.toys";
		case TOY_ID:
			return "vnd.android.cursor.item/vnd.biz.varkon.shelvesom.toys";
		default:
			throw new IllegalArgumentException("Unknown URI " + uri);
		}
	}
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (URI_MATCHER.match(uri) != TOYS) {
			throw new IllegalArgumentException("Unknown URI " + uri);
		}

		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
		final long rowId = db.insert("toys", BaseItem.TITLE, values);
		if (rowId > 0) {
			Uri insertUri = ContentUris.withAppendedId(
					ToysStore.Toy.CONTENT_URI, rowId);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (URI_MATCHER.match(uri)) {
		case TOYS:
			count = db.delete("toys", selection, selectionArgs);
			break;
		case TOY_ID:
			String segment = uri.getPathSegments().get(1);
			count = db.delete("toys", BaseItem._ID
					+ "="
					+ segment
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch (URI_MATCHER.match(uri)) {
		case TOYS:
			count = db.update("toys", values, selection, selectionArgs);
			break;
		case TOY_ID:
			count = db.update("toys", values,
					BaseItem._ID
							+ " = "
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE toys (" + BaseItem._ID
			...
			db.execSQL("CREATE INDEX toyIndexTitle ON toys("
					+ BaseItem.SORT_TITLE + ");");
			db.execSQL("CREATE INDEX toyIndexAuthors ON toys("
					+ BaseItem.AUTHORS + ");");
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			switch (oldVersion) {
			case 1:
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.LOANED_TO
						+ " TEXT");
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.LOAN_DATE
						+ " TEXT");
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.EVENT_ID
						+ " INTEGER");
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.NOTES
						+ " TEXT");
			case 2:
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.UPC
						+ " TEXT");
			case 3:
				db.execSQL("ALTER TABLE toys ADD COLUMN "
						+ BaseItem.WISHLIST_DATE + " TEXT");
			case 4:
				db.execSQL("ALTER TABLE toys ADD COLUMN " + BaseItem.QUANTITY
						+ " TEXT");
				break;
			default:
				break;
