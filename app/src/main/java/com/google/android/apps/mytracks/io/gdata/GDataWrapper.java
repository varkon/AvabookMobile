 ...
 * 
 * @param C
 *            the GData service client
 * @author Sandor Dornbush
 */
public class GDataWrapper<C> {
	private static final String LOG_TAG = "GDataWrapper";
	 ...
	 * of the class is called with the GData client as a parameter. The function
	 * should execute whatever operations it desires on the client without
	 ...
	 * with the current authentication token for the service. The function
	 * should execute whatever operations with the google service without
	 * concern for whether the client will throw an error.
	 */
	...
	public boolean runAuthenticatedFunction(final AuthenticatedFunction function) {
	...
	private boolean runCommon(final AuthenticatedFunction function,
		...
		for (int i = 0; i <= AUTH_TOKEN_INVALIDATE_REFRESH_NUM_RETRIES; i++) {
			...
			Log.d(LOG_TAG, "GData error encountered: " + errorMessage);
			...
			Log.d(LOG_TAG, "retrying function/query");
		}
		return false;
	}
	...
	private void runOne(final AuthenticatedFunction function,
		...
		try {
			...
			Log.e(LOG_TAG, "HttpException, code " + e.getStatusCode()
